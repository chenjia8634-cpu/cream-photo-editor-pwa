...
视频导出使用浏览器本地的 ffmpeg.wasm 转码引擎。首次处理视频会加载视频引擎模块并下载约 30MB 的转码核心，页面会显示模块加载阶段、实时下载进度和初始化等待时间，之后由浏览器缓存。建议优先处理 Live Photo 这种几秒钟的短视频。工具会尽量保留原视频声音；如果原视频音频轨道无法直接复制，会改用 AAC 音频重新编码。
