...
一个纯前端 MVP：用户在 iPhone 浏览器中上传一张或多张照片，浏览器本地使用 Canvas API 完成固定预设调色，预览后保存 JPG 图片。照片不会上传到服务器。
...
3. 上传一张或多张 JPG、PNG、HEIC 或 HEIF 测试预览和保存。
...
1. 打开网页并上传一张或多张 JPG、JPEG、PNG、HEIC 或 HEIF。
2. 等待批量结果列表出现。
3. 单张保存：点击每张结果旁边的 `下载`，或点击底部 `保存高质量图片` 保存最后一张。
4. 批量保存：点击底部 `保存全部图片`。
5. 如果出现系统分享面板，可以选择保存到照片或文件。
6. 如果浏览器走下载逻辑，图片会按顺序下载为 JPG 文件，可在下载内容中打开后保存。
